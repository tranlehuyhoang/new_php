...
$cat = new code();
$catId = $_GET['editid'];
$getcatbyid = $cat->getcodebyid($catId);
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['editid'])) {
    $updatecat = $cat->update_code($_POST, $_GET['editid']);
    $getcatbyid = $cat->getcodebyid($catId);
}
?>
                ...
                <div class="col-xl-6">
                    <div class="card custom-card">
                        <div class="card-header justify-content-between">
                            <div class="card-title"> Create Code Form </div>
                            <?php
                            if (isset($updatecat)) {
                                echo $updatecat;
                            }
                        ...
                        ?>
                            <div class="card-body">
                                <div class="mb-3"> <label for="form-text" class="form-label fs-14 text-dark">Nhập mã giảm
                                        giá
                                    </label> <input name="codename" value="<?php echo $result['codename'] ?>" type="text" class="form-control" id="form-text" placeholder="">
                                </div>
                                <div class="mb-3"> <label for="form-text" class="form-label fs-14 text-dark">Nhập %
                                    </label>
                                    <select name="codepercent" class="form-select" id="selectform" multiple aria-label="multiple select example">
                                        <?php
                                        for ($a = 0; $a <= 100; $a += 5) {
                                            echo '<option ';
                                            if ($result['codepercent'] == $a) {
                                                echo 'selected ';
                                            }
