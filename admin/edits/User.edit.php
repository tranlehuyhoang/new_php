...
$cat = new user();
$catId = $_GET['editid'];
$getcatbyid = $cat->getuserbyid($catId);
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['editid'])) {
    $updatecat = $cat->update_user($_POST, $_GET['editid']);
    $getcatbyid = $cat->getuserbyid($catId);
}
?>
            ...
            <div class="ms-md-1 ms-0">
                <nav>
                    <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item"><a href="catlist.php">User</a></li>
            ...
            <div class="row">
                <div class="col-xl-6">
                    <div class="card custom-card">
                        ...
                        ?>
                            <div class="card-body">
                                <div class="mb-3"> <label for="form-text" class="form-label fs-14 text-dark"> tên user
                                    </label> <input value="<?php echo $result['username'] ?>" name="username" type="text" class="form-control" id="form-text" placeholder="" disabled>
                                </div>
                                <div class="mb-3"> <label for="form-text" class="form-label fs-14 text-dark"> email user
                                    </label> <input value="<?php echo $result['useremail'] ?>" name="useremail" type="email" class="form-control" id="form-text" placeholder="" disabled>
                                </div>
                                <div class="mb-3"> <label for="form-text" class="form-label fs-14 text-dark"> pass user
                                    </label> <input value="<?php echo $result['userpass'] ?>" name="userpass" type="password" class="form-control" id="form-text" placeholder="" disabled>
                                </div>
                                <div class="mb-3">
                                    <label for="selectform" class="form-label fs-14 text-dark">Chọn quyền
                                    </label>
                                    <select class="form-select" id="selectform" name="userroles" aria-label="multiple select example">

                                        <option <?php
                                                if ($result['userroles'] == '1') {
                                                    echo 'selected ';
                                                }
                                                ?> value="1">USER</option>
                                        <option <?php
                                                if ($result['userroles'] == '2') {
                                                    echo 'selected ';
                                                }
                                                ?>value="2">ADMIN</option>
                                        <option <?php
                                                if ($result['userroles'] == '3') {
                                                    echo 'selected ';
                                                }
                                                ?> value="3">GUEST</option>
