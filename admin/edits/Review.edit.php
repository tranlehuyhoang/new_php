...
$catId = $_GET['editid'];
$getcatbyid = $cat->getreviewbyid($catId);
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['editid'])) {
    $updatecat = $cat->update_review($_POST, $_GET['editid']);
    $getcatbyid = $cat->getreviewbyid($catId);
}
                        ...
                        ?>
                            <div class="card-body">
                                    ...
                                    <select class="form-select" id="selectform" name="reviewuserid" aria-label="multiple select example">
                                        ...
                                        if (isset($show)) {
                                            if ($show && $show->num_rows > 0) {
                                                $i = 0;
                                                while ($resultsss = $show->fetch_assoc()) {
                                                    # code...
                                        ?>
                                                    <option <?php

                                                            if ($results['reviewuserid'] == $resultsss['userid']) {
                                                                echo 'selected ';
                                                            }
                                                            ?> value="<?php echo $resultsss['userid']; ?>">
                                                        <?php echo $resultsss['userid']; ?> :
                                                        <?php echo $resultsss['useremail']; ?> </option>
                                ...
                                <div class="mb-3"> <label for="form-text" class="form-label fs-14 text-dark">Nhập số sao
                                    </label> <input value="<?php echo $results['reviewrating'] ?>" name="reviewrating" min="0" value="0" max="5" type="number" class="form-control" id="form-text" placeholder="">
                                </div>
                                <div class="mb-3"> <label for="form-text" class="form-label fs-14 text-dark">Chọn Sản phẩm
                                        ...
                                        if (isset($shows)) {
                                            if ($shows && $shows->num_rows > 0) {
                                                $i = 0;
                                                while ($resultaa = $shows->fetch_assoc()) {
                                                    # code...
                                        ?>
                                                    <option <?php

                                                            if ($results['reviewproductid'] == $resultaa['productid']) {
                                                                echo 'selected ';
                                                            }
                                                            ?> value="<?php echo  $resultaa['productid']; ?>">
                                                        <?php echo  $resultaa['productid']; ?> :
                                                        <?php echo  $resultaa['productname']; ?>
                                                    </option>
